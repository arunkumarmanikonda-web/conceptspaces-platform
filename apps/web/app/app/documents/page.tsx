...
import CDEWorkspace,{type CDEDocument,type CDEModel,type CDETransmittal} from "@/components/CDEWorkspace";

export const dynamic="force-dynamic";

type Project={id:string;code:string;name:string};
type CDEData={documents?:CDEDocument[];models?:CDEModel[];transmittals?:CDETransmittal[]};
...
export default async function DocumentsPage({searchParams}:{searchParams:Promise<{project?:string}>}){
  ...
  const documents=cde.documents||[];
  const models=cde.models||[];
  const transmittals=cde.transmittals||[];
  const review=documents.filter(d=>d.status==="for_review").length;
  const approval=documents.filter(d=>d.status==="for_approval").length;
