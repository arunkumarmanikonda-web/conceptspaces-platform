...
const initial:FormState={
  clientName:"",email:"",organisation:"",phone:"",projectName:"",gstRegistered:"",gstin:"",billingLegalName:"",billingAddress:"",billingState:"",address:"",latitude:"",longitude:"",siteArrangement:"single",combinedAreaUnit:"sqyd",
...
export function ProjectIntakeWizardLive(){
  ...
  useEffect(()=>{
    try{
      ...
      if(stored){
        ...
        if(draft.form&&typeof draft.form==="object") setForm(previous=>{
          const restored={...previous,...draft.form};
          if(!["","yes","no"].includes(restored.gstRegistered)) restored.gstRegistered="";
          if(!arrangementOptions.some(option=>option.value===restored.siteArrangement)) restored.siteArrangement="single";
          return restored;
        });
  ...
  const setGstRegistration=(value:string)=>setForm(previous=>({...previous,gstRegistered:value,gstin:value==="yes"?previous.gstin:""}));
  ...
  const validationForStep=(target:number):string|null=>{
    if(target===0){
      if(!form.clientName.trim()||!form.projectName.trim()) return "Enter the client / owner name and project name.";
      if(!["yes","no"].includes(form.gstRegistered)) return "Select whether the client is GST registered.";
      if(form.gstRegistered==="yes"&&(!form.billingLegalName.trim()||!form.billingAddress.trim()||!form.billingState.trim())) return "Enter the registered legal name, billing address and billing state / UT.";
      if(form.gstRegistered==="yes"&&!isValidGstin(form.gstin)) return "Enter a structurally valid 15-character GSTIN.";
    }
    if(target===1){
  ...
  return <div className="wizard">
    ...
    <section className="form">
      ...
      {step===0&&<><Grid><Field label="Client / owner name" field="clientName" form={form} set={set} required/><Field label="Email" field="email" form={form} set={set} type="email"/><Field label="Organisation" field="organisation" form={form} set={set}/><Field label="Phone" field="phone" form={form} set={set} type="tel"/><Field label="Project name" field="projectName" form={form} set={set} required/></Grid>
        <div className="section-heading"><div><h2>Billing identity</h2><p className="subtle">Capture the invoice identity now. GST details remain client-declared until verified.</p></div></div>
        <Grid><SelectField label="GST registered?" field="gstRegistered" form={form} onValueChange={setGstRegistration} required options={[{value:"",label:"Select yes or no"},{value:"yes",label:"Yes"},{value:"no",label:"No"}]}/><Field label="Legal billing name" field="billingLegalName" form={form} set={set} required={form.gstRegistered==="yes"}/>{form.gstRegistered==="yes"&&<Field label="GSTIN" field="gstin" form={form} set={set} required transform={normaliseGstin} maxLength={15} placeholder="15-character GSTIN"/>}<Area label="Billing address" field="billingAddress" form={form} set={set} required={form.gstRegistered==="yes"}/><Field label="Billing state / UT" field="billingState" form={form} set={set} required={form.gstRegistered==="yes"}/></Grid>
        {form.gstRegistered==="yes"&&<div className="note"><b>GST verification.</b> The GSTIN is checked for structure only. Registry verification is still required before issuing a tax invoice.</div>}</>}

      {step===1&&<><Grid><Field label="Site address" field="address" form={form} set={set} required/><SelectField label="Site composition" field="siteArrangement" form={form} onValueChange={setArrangement} options={arrangementOptions}/><Field label="Latitude" field="latitude" form={form} set={set}/><Field label="Longitude" field="longitude" form={form} set={set}/></Grid>
...
function Field({label,field,form,set,type="text",placeholder,required,transform,maxLength}:{label:string;field:string;form:FormState;set:(field:string,value:string)=>void;type?:string;placeholder?:string;required?:boolean;transform?:(value:string)=>string;maxLength?:number}){return <div className="field"><label htmlFor={`intake-${field}`}>{label}{required?" *":""}</label><input id={`intake-${field}`} type={type} min={type==="number"?"0":undefined} step={type==="number"?"any":undefined} maxLength={maxLength} value={form[field]||""} placeholder={placeholder} required={required} onChange={event=>set(field,transform?transform(event.target.value):event.target.value)}/></div>}
...
function SelectField({label,field,form,onValueChange,options,required}:{label:string;field:string;form:FormState;onValueChange:(value:string)=>void;options:{value:string;label:string}[];required?:boolean}){return <div className="field"><label htmlFor={`intake-${field}`}>{label}{required?" *":""}</label><select id={`intake-${field}`} value={form[field]||options[0]?.value||""} required={required} onChange={event=>onValueChange(event.target.value)}>{options.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></div>}
