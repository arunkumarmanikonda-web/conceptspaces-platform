...
grant select,insert,update on public.document_templates,public.template_versions,public.project_report_snapshots,public.generation_jobs,public.generated_artifacts,public.publication_sets,public.publication_set_items to authenticated;

create policy document_templates_runtime_insert on public.document_templates for insert to authenticated
with check (core.has_org_role(organisation_id,array['super_admin','org_admin','project_manager','lead_architect']));
create policy template_versions_runtime_insert on public.template_versions for insert to authenticated
with check (exists(select 1 from public.document_templates t where t.id=template_id and core.has_org_role(t.organisation_id,array['super_admin','org_admin','project_manager','lead_architect'])));
create policy project_report_snapshots_runtime_insert on public.project_report_snapshots for insert to authenticated
with check (created_by=(select auth.uid()) and project.can_manage_project(project_id));
create policy generation_jobs_runtime_insert on public.generation_jobs for insert to authenticated
with check (requested_by=(select auth.uid()) and project_id is not null and project.can_manage_project(project_id));
create policy generation_jobs_runtime_update on public.generation_jobs for update to authenticated
using (requested_by=(select auth.uid()) and project_id is not null and project.can_manage_project(project_id))
with check (requested_by=(select auth.uid()) and project_id is not null and project.can_manage_project(project_id));
create policy generated_artifacts_runtime_insert on public.generated_artifacts for insert to authenticated
with check (project_id is not null and project.can_manage_project(project_id));
create policy publication_sets_runtime_insert on public.publication_sets for insert to authenticated
with check (project.can_manage_project(project_id));
create policy publication_sets_runtime_update on public.publication_sets for update to authenticated
using (project.can_manage_project(project_id)) with check (project.can_manage_project(project_id));
create policy publication_set_items_runtime_insert on public.publication_set_items for insert to authenticated
with check (exists(select 1 from public.publication_sets p where p.id=publication_set_id and project.can_manage_project(p.project_id)));
...
language plpgsql
security invoker
set search_path=public,project,regula,cde,core,audit,extensions
as $$
declare
...
begin
  ...
  if not project.can_manage_project(target_project_id) then raise exception 'project_manage_authority_required'; end if;
  ...
  ) order by f.checked_at nulls last),'[]'::jsonb) into regula_payload
  from regula.compliance_findings f where f.project_id=p.id;

  commercial_payload:=jsonb_build_object(
    'contracts',coalesce((select jsonb_agg(jsonb_build_object('id',c.id,'status',c.status,'version',c.version,'effective_at',c.effective_at,'expires_at',c.expires_at) order by c.created_at) from public.contracts c where c.project_id=p.id),'[]'::jsonb),
    'invoices',coalesce((select jsonb_agg(jsonb_build_object('id',i.id,'invoice_number',i.invoice_number,'status',i.status,'currency',i.currency,'issue_date',i.issue_date,'due_date',i.due_date,'subtotal',i.subtotal,'tax',i.tax,'total',i.total,'amount_paid',i.amount_paid,'tds_receivable',i.tds_receivable) order by i.issue_date) from public.invoices i where i.project_id=p.id),'[]'::jsonb)
  );

  document_payload:=jsonb_build_object(
    'documents',coalesce((select jsonb_agg(jsonb_build_object('id',d.id,'document_number',d.document_number::text,'title',d.title,'discipline',d.discipline,'document_type',d.document_type,'cde_state',d.cde_state,'status',d.status,'revision',d.revision,'current_version_id',d.current_version_id) order by d.document_number,d.revision) from cde.documents d where d.project_id=p.id),'[]'::jsonb),
    'models',coalesce((select jsonb_agg(jsonb_build_object('id',m.id,'model_name',m.model_name,'discipline',m.discipline,'format',m.format,'schema_version',m.schema_version,'status',m.status,'checksum',m.checksum) order by m.created_at) from cde.models m where m.project_id=p.id),'[]'::jsonb)
  );
  ...
  insert into public.project_report_snapshots(project_id,report_type,project_truth_hash,requirement_snapshot_hash,regulatory_snapshot_hash,commercial_snapshot_hash,document_snapshot_hash,source_refs,snapshot_payload,created_by)
  values(p.id,report_value,truth_hash,requirement_hash,regula_hash,commercial_hash,document_hash,
    jsonb_build_array(jsonb_build_object('type','project_truth','hash',truth_hash),jsonb_build_object('type','requirements','hash',requirement_hash),jsonb_build_object('type','regulatory','hash',regula_hash),jsonb_build_object('type','commercial','hash',commercial_hash),jsonb_build_object('type','documents','hash',document_hash)),payload,auth.uid())
  ...
  on conflict (organisation_id,code) do update set active=true,updated_at=now()
  returning id into template_id;

  select tv.id into template_version_id from public.template_versions tv where tv.template_id=template_id and tv.version=1;
  if template_version_id is null then
    insert into public.template_versions(template_id,version,schema_version,template_ref,checksum,required_data_paths,locked,created_by,approved_by,approved_at)
    values(template_id,1,'conceptspaces.project-report.v1','runtime:conceptspaces-project-report-v1',template_checksum,'["project","project_truth","requirements"]'::jsonb,true,auth.uid(),auth.uid(),now())
  ...
  values(org_id,p.id,template_version_id,format_value,'running',snapshot_id,input_hash,auth.uid(),now()) returning id into job_id;
...
$$;
revoke all on function public.prepare_project_generation(uuid,text,text) from public,anon;
...
language plpgsql
security invoker
set search_path=public,project,audit
as $$
declare j public.generation_jobs%rowtype; a public.generated_artifacts%rowtype;
begin
  ...
  if j.requested_by<>auth.uid() and not project.can_manage_project(j.project_id) then raise exception 'generation_job_authority_required'; end if;
...
$$;
revoke all on function public.complete_project_generation(uuid,text,text,text) from public,anon;
...
begin
  ...
  perform audit.append_event(j.organisation_id,j.project_id,'report.generation.failed','generation_job',j.id,null,jsonb_build_object('error_code',target_error_code),target_error_code,gen_random_uuid());
end;
$$;
revoke all on function public.fail_project_generation(uuid,text) from public,anon;
...
language sql stable security invoker set search_path=public,project
as $$
  select a.id,a.title,a.output_format,a.object_ref,a.checksum,a.status,a.revision,a.source_snapshot_id,a.created_at
  from public.generated_artifacts a where a.project_id=target_project_id and project.can_access_project(target_project_id)
  order by a.created_at desc;
$$;
revoke all on function public.list_project_generated_artifacts(uuid) from public,anon;
