...
grant select,insert,update on public.project_branches to authenticated;
grant select,insert on public.project_commits to authenticated;
grant select,insert,update on public.change_impacts to authenticated;
...
returns text
language plpgsql security invoker
set search_path=public,project,coordination,audit,auth,pg_temp
as $$
...
begin
  ...
  select * into approval from coordination.approval_requests where id=req.approval_request_id; if not found then raise exception 'approval_request_not_found'; end if;
  if approval.decision='pending' then return 'pending'; end if;
  before_state:=to_jsonb(req); perform set_config('conceptspaces.configuration_phase','sync_approval',true);
...
begin
  ...
  before_branch:=to_jsonb(b); before_req:=to_jsonb(req); perform set_config('conceptspaces.configuration_phase','commit',true);
  insert into public.project_commits(project_id,branch_id,parent_commit_hashes,content_hash,message,changed_object_refs,author_type,author_ref) values(req.project_id,b.id,jsonb_build_array(b.head_commit_hash),commit_hash,coalesce(nullif(btrim(target_message),''),req.change_ref||' · '||req.title),req.source_object_refs,'human',auth.uid()::text);
  update public.project_branches set head_commit_hash=commit_hash where id=b.id returning * into b;
  perform set_config('conceptspaces.configuration_phase','apply',true); update public.project_change_requests set status='applied',applied_by=auth.uid(),applied_at=now(),updated_at=now() where id=req.id returning * into req;
  select organisation_id into org_id from project.projects where id=req.project_id; perform audit.append_event(org_id,req.project_id,'configuration.change.applied','project_change_request',req.id,before_req,to_jsonb(req),commit_hash,gen_random_uuid()); perform audit.append_event(org_id,req.project_id,'configuration.branch.commit','project_branch',b.id,before_branch,to_jsonb(b),commit_hash,gen_random_uuid());
  return commit_hash;
end;$$;
...
begin
  ...
  select * into req from public.project_change_requests where id=target_change_request_id and project_id=source_branch.project_id and branch_id=source_branch.id; if not found or req.status<>'applied' or req.latest_impact_id is null then raise exception 'applied_source_branch_change_required'; end if;
  ...
  if configuration.project_configuration_hash(source_branch.project_id)<>impact.baseline_hash then raise exception 'project_changed_reanalysis_required'; end if;
  ...
  before_source:=to_jsonb(source_branch); before_target:=to_jsonb(target_branch); perform set_config('conceptspaces.configuration_phase','merge',true);
  insert into public.project_commits(project_id,branch_id,parent_commit_hashes,content_hash,message,changed_object_refs,author_type,author_ref) values(source_branch.project_id,target_branch.id,jsonb_build_array(target_branch.head_commit_hash,source_branch.head_commit_hash),merge_hash,'Merge '||source_branch.name||' → '||target_branch.name||': '||btrim(target_reason),req.source_object_refs,'human',auth.uid()::text);
  update public.project_branches set head_commit_hash=merge_hash where id=target_branch.id returning * into target_branch; update public.project_branches set status='merged' where id=source_branch.id returning * into source_branch;
  select organisation_id into org_id from project.projects where id=source_branch.project_id; perform audit.append_event(org_id,source_branch.project_id,'configuration.branch.merged','project_branch',source_branch.id,before_source,to_jsonb(source_branch),merge_hash,gen_random_uuid()); perform audit.append_event(org_id,target_branch.project_id,'configuration.branch.merge_commit','project_branch',target_branch.id,before_target,to_jsonb(target_branch),merge_hash,gen_random_uuid());
  return merge_hash;
end;$$;
...
create or replace function public.cancel_project_change(target_change_request_id uuid,target_reason text)
returns void
language plpgsql security invoker
set search_path=public,project,audit,auth,pg_temp
as $$
declare req public.project_change_requests%rowtype; before_state jsonb; org_id uuid;
begin
  select * into req from public.project_change_requests where id=target_change_request_id for update; if not found or auth.uid() is null or not project.can_access_project(req.project_id) then raise exception 'change_request_access_required'; end if;
  if req.status in ('applied','cancelled') then raise exception 'terminal_change_request_state'; end if; if nullif(btrim(target_reason),'') is null then raise exception 'cancel_reason_required'; end if;
  before_state:=to_jsonb(req); perform set_config('conceptspaces.configuration_phase','cancel',true); update public.project_change_requests set status='cancelled',updated_at=now() where id=req.id returning * into req;
  select organisation_id into org_id from project.projects where id=req.project_id; perform audit.append_event(org_id,req.project_id,'configuration.change.cancelled','project_change_request',req.id,before_state,to_jsonb(req),btrim(target_reason),gen_random_uuid());
end;$$;
revoke all on function public.cancel_project_change(uuid,text) from public,anon;
grant execute on function public.cancel_project_change(uuid,text) to authenticated;
