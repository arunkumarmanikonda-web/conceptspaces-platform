...
type Version={id:string;version:number;object_key:string;mime_type:string;size_bytes:number;checksum:string;created_at:string};
type DocumentRow={id:string;document_number:string;title:string;discipline:string;document_type:string;cde_state:string;status:string;revision:string;current_version_id?:string|null;current_version?:Version|null;updated_at:string};
type ModelRow={id:string;model_name:string;discipline:string;format:string;schema_version?:string|null;object_key:string;checksum:string;status:string;created_at:string};
type TransmittalRow={id:string;transmittal_number:string;message?:string|null;issued_at?:string|null;created_at:string;item_count:number};

type Props={projectId:string;documents:DocumentRow[];models:ModelRow[];transmittals:TransmittalRow[]};

function safeName(value:string){return value.normalize("NFKD").replace(/[^a-zA-Z0-9._-]+/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"").slice(0,120)||"file";}
async function sha256(file:File){const buffer=await file.arrayBuffer();const digest=await crypto.subtle.digest("SHA-256",buffer);return Array.from(new Uint8Array(digest)).map(b=>b.toString(16).padStart(2,"0")).join("");}
...
export default function CDEWorkspace({projectId,documents,models,transmittals}:Props){
  ...
  async function registerDocument(formData:FormData){
    ...
    const objectKey=`${projectId}/documents/${documentId}/v1/${safeName(file.name)}`;
    ...
    if(uploadError){setMessage(`Private upload failed: ${uploadError.message}`);return;}
    const {error:versionError}=await supabase.rpc("register_cde_file_version",{target_document_id:documentId,object_key:objectKey,mime_type:file.type||"application/octet-stream",size_bytes:file.size,checksum_sha256:checksum});
    if(versionError){await supabase.storage.from("project-cde").remove([objectKey]);setMessage(`Version registration failed and upload was rolled back: ${versionError.message}`);return;}
  ...
  async function downloadVersion(documentId:string){
    const doc=documents.find(d=>d.id===documentId);const key=doc?.current_version?.object_key;if(!key)return;
  ...
  async function requestApproval(documentId:string){
    const doc=documents.find(d=>d.id===documentId);const hash=doc?.current_version?.checksum;if(!doc||!hash){setMessage("A registered file version is required first.");return;}
    const {error}=await supabase.rpc("request_governed_approval",{input_payload:{project_id:projectId,resource_type:"document",resource_id:documentId,role_required:"lead_architect",criticality:"C3",comments:`Review exact SHA-256 ${hash}`}});
    if(error){setMessage(error.message);return;}refresh("C3 document approval requested against the current SHA-256 hash.");
  }
  ...
  async function uploadModel(formData:FormData){
    ...
    const format=(file.name.split(".").pop()||String(formData.get("format")||"")).toLowerCase();
