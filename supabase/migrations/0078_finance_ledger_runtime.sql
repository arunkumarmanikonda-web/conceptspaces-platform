...
grant select,insert on finance.ledger_accounts to authenticated;
grant select,insert,update on finance.journals to authenticated;
grant select,insert on finance.journal_lines to authenticated;

drop policy if exists fiscal_periods_read on finance.fiscal_periods;
create policy fiscal_periods_read on finance.fiscal_periods
...
drop policy if exists fiscal_periods_insert on finance.fiscal_periods;
create policy fiscal_periods_insert on finance.fiscal_periods
for insert to authenticated with check(
 core.has_org_role(organisation_id,array['super_admin','org_admin','finance'])
 and current_setting('conceptspaces.finance_phase',true)='period'
);
drop policy if exists fiscal_periods_update on finance.fiscal_periods;
create policy fiscal_periods_update on finance.fiscal_periods
...
create policy ledger_accounts_write on finance.ledger_accounts
for insert to authenticated with check(
 core.has_org_role(organisation_id,array['super_admin','org_admin','finance'])
 and current_setting('conceptspaces.finance_phase',true)='account'
);

drop policy if exists journals_write on finance.journals;
create policy journals_write on finance.journals
for insert to authenticated with check(
 core.has_org_role(organisation_id,array['super_admin','org_admin','finance'])
 and current_setting('conceptspaces.finance_phase',true) in ('journal_create','journal_reversal')
);
...
with check(
 core.has_org_role(organisation_id,array['super_admin','org_admin','finance'])
 and current_setting('conceptspaces.finance_phase',true)='journal_post'
);

drop policy if exists journal_lines_write on finance.journal_lines;
create policy journal_lines_write on finance.journal_lines
for insert to authenticated with check(
 exists(
  select 1 from finance.journals j
  where j.id=journal_lines.journal_id
    and core.has_org_role(j.organisation_id,array['super_admin','org_admin','finance'])
    and current_setting('conceptspaces.finance_phase',true) in ('journal_create','journal_reversal')
 )
);
...
begin
 ...
 if tg_op='UPDATE' and old.status in ('posted','reversed') then
   ...
   if (to_jsonb(new)-'status'-'reversal_reason'-'updated_at') is distinct from (to_jsonb(old)-'status'-'reversal_reason'-'updated_at') then
     raise exception 'posted_journal_content_immutable';
   end if;
 end if;
...
language plpgsql
security invoker
set search_path='public','finance','core','audit','auth','pg_temp'
as $$
...
begin
 ...
 if nullif(input_payload->>'parent_id','') is not null and not exists(
   select 1 from finance.ledger_accounts a where a.id=(input_payload->>'parent_id')::uuid and a.organisation_id=target_organisation_id
 ) then raise exception 'parent_account_invalid'; end if;
 ...
 values(target_organisation_id,upper(btrim(input_payload->>'code')),btrim(input_payload->>'name'),type_value,nullif(input_payload->>'parent_id','')::uuid,nullif(upper(btrim(input_payload->>'currency')),''),true,coalesce(input_payload->'metadata','{}'::jsonb))
...
begin
 ...
 perform set_config('conceptspaces.finance_phase','period',true);
 insert into finance.fiscal_periods(organisation_id,period_code,start_date,end_date,status,created_by)
 values(target_organisation_id,upper(btrim(input_payload->>'period_code')),start_value,end_value,'open',auth.uid()) returning * into period;
 perform audit.append_event(target_organisation_id,null,'finance.period.created','fiscal_period',period.id,null,to_jsonb(period),null,gen_random_uuid());
 return period.id;
end;
$$;
...
begin
 ...
 if value=period.status then return period.status; end if;
 if period.status='closed' and value='open' and not core.has_org_role(period.organisation_id,array['super_admin','org_admin']) then raise exception 'period_reopen_admin_authority_required'; end if;
 before_state:=to_jsonb(period);
 perform set_config('conceptspaces.finance_phase','period',true);
 update finance.fiscal_periods
 set status=value,closed_by=case when value='closed' then auth.uid() else null end,closed_at=case when value='closed' then now() else null end,updated_at=now()
 where id=period.id returning * into period;
 perform audit.append_event(period.organisation_id,null,'finance.period.'||value,'fiscal_period',period.id,before_state,to_jsonb(period),target_reason,gen_random_uuid());
...
language plpgsql
security invoker
set search_path='public','finance','project','core','audit','auth','pg_temp'
as $$
declare
 journal finance.journals%rowtype;
 line jsonb;
 debit_total numeric:=0;
 credit_total numeric:=0;
 debit_value numeric;
 credit_value numeric;
 account finance.ledger_accounts%rowtype;
 project_id_value uuid;
 tax_id_value uuid;
 posting_value date:=coalesce(nullif(input_payload->>'posting_date','')::date,current_date);
 number_value text;
begin
 if auth.uid() is null or not core.has_org_role(target_organisation_id,array['super_admin','org_admin','finance']) then raise exception 'finance_authority_required'; end if;
 ...
 values(target_organisation_id,number_value,posting_value,'draft',lower(btrim(input_payload->>'source_type')),nullif(input_payload->>'source_id','')::uuid,auth.uid(),nullif(btrim(input_payload->>'memo'),''))
 ...
 for line in select value from jsonb_array_elements(input_payload->'lines') loop
   ...
   tax_id_value:=nullif(line->>'tax_determination_id','')::uuid;
   if tax_id_value is not null and not exists(select 1 from finance.tax_determinations t where t.id=tax_id_value and t.organisation_id=target_organisation_id) then raise exception 'journal_tax_determination_invalid'; end if;
   ...
   values(journal.id,account.id,project_id_value,nullif(btrim(line->>'cost_code'),''),nullif(line->>'counterparty_id','')::uuid,debit_value,credit_value,coalesce(nullif(upper(btrim(line->>'currency')),''),coalesce(account.currency,'INR')),tax_id_value,nullif(btrim(line->>'memo'),''));
 ...
 perform audit.append_event(target_organisation_id,null,'finance.journal.created','journal',journal.id,null,jsonb_build_object('journal',to_jsonb(journal),'debit_total',debit_total,'credit_total',credit_total),null,gen_random_uuid());
 return journal.id;
end;
$$;
...
language plpgsql
security invoker
set search_path='public','finance','core','audit','extensions','auth','pg_temp'
as $$
...
begin
 ...
 select * into period from finance.fiscal_periods
 where organisation_id=journal.organisation_id and journal.posting_date between start_date and end_date and status='open'
 ...
 hash_value:=encode(extensions.digest(jsonb_build_object(
   ...
   'memo',journal.memo,
   'lines',(select coalesce(jsonb_agg(to_jsonb(l) order by l.id),'[]'::jsonb) from finance.journal_lines l where l.journal_id=journal.id)
 )::text,'sha256'),'hex');
 before_state:=to_jsonb(journal);
 perform set_config('conceptspaces.finance_phase','journal_post',true);
 update finance.journals set status='posted',posted_by=auth.uid(),posted_at=now(),posted_hash=hash_value,fiscal_period_id=period.id,updated_at=now()
 where id=journal.id returning * into journal;
 if journal.reversal_of_id is not null then
   update finance.journals set status='reversed',reversal_reason=target_reason,updated_at=now() where id=journal.reversal_of_id;
 end if;
 perform audit.append_event(journal.organisation_id,null,'finance.journal.posted','journal',journal.id,before_state,to_jsonb(journal),target_reason,gen_random_uuid());
...
begin
 ...
 values(original.organisation_id,number_value,posting_value,'draft','reversal',original.id,original.id,auth.uid(),'Reversal of '||original.journal_number,target_reason)
 ...
 perform audit.append_event(original.organisation_id,null,'finance.journal.reversal_created','journal',reversal.id,null,to_jsonb(reversal),target_reason,gen_random_uuid());
 return reversal.id;
end;
$$;
...
stable
security invoker
set search_path='public','finance','project','core','auth','pg_temp'
as $$
begin
 ...
 return jsonb_build_object(
  ...
  'summary',jsonb_build_object(
    ...
    'issued_invoice_total',coalesce((select sum(i.total) from public.invoices i where i.organisation_id=target_organisation_id and i.status in ('issued','part_paid','paid','overdue')),0),
    'posted_journal_count',(select count(*) from finance.journals j where j.organisation_id=target_organisation_id and j.status='posted'),
    'draft_journal_count',(select count(*) from finance.journals j where j.organisation_id=target_organisation_id and j.status='draft')
  )
 );
end;
$$;
