...
function SelectField({label,field,form,set,options,required}:{label:string;field:string;form:FormState;set:(field:string,value:string)=>void;options:{value:string;label:string}[];required?:boolean}){return <div className="field"><label htmlFor={`intake-${field}`}>{label}{required?" *":""}</label><select id={`intake-${field}`} value={form[field]||options[0]?.value||""} required={required} onChange={event=>set(field,event.target.value)}>{options.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></div>}
