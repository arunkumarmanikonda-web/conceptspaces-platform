...
-- Controlled site packages are immutable snapshots of approved/issued project information.
create or replace function site.current_controlled_snapshot(target_project_id uuid)
returns jsonb
language sql stable security invoker
set search_path=site,cde,project,pg_temp
as $$
...
-- Permit sync RPCs to write diaries and narrowly link auto-created NCRs.
drop policy if exists site_diaries_insert on site.site_diaries;
create policy site_diaries_insert on site.site_diaries for insert to authenticated with check(project.can_access_project(project_id) and captured_by=auth.uid() and current_setting('conceptspaces.site_phase',true) in ('diary','offline_sync'));
drop policy if exists inspection_site_update on public.inspection_records;
create policy inspection_site_update on public.inspection_records for update to authenticated
using((current_setting('conceptspaces.site_phase',true)='inspection_review' and project.can_manage_project(project_id)) or (current_setting('conceptspaces.site_phase',true)='inspection_link' and project.can_access_project(project_id)))
with check((current_setting('conceptspaces.site_phase',true)='inspection_review' and project.can_manage_project(project_id)) or (current_setting('conceptspaces.site_phase',true)='inspection_link' and project.can_access_project(project_id)));

-- Progress claims are mutable only through governed claim phases.
drop policy if exists progress_claims_site_insert on public.progress_claims;
...
drop policy if exists progress_claims_site_update on public.progress_claims;
...
begin
  ...
  insert into site.offline_packages(project_id,user_id,source_snapshot,source_hash,expires_at,status)
  values(target_project_id,auth.uid(),snapshot,hash_value,target_expires_at,'active') returning * into p;
...
as $$
declare p site.offline_packages%rowtype; snapshot jsonb; current_hash text;
begin
  ...
  snapshot:=site.current_controlled_snapshot(p.project_id); current_hash:=encode(extensions.digest(snapshot::text,'sha256'),'hex');
...
as $$
declare d site.site_diaries%rowtype; org_id uuid; offline_id uuid:=nullif(input_payload->>'offline_package_id','')::uuid;
begin
  if auth.uid() is null or not project.can_access_project(target_project_id) then raise exception 'project_access_required'; end if;
  if offline_id is not null and not exists(select 1 from site.offline_packages p where p.id=offline_id and p.project_id=target_project_id and p.user_id=auth.uid()) then raise exception 'offline_package_access_denied'; end if;
  if current_setting('conceptspaces.site_phase',true)<>'offline_sync' then perform set_config('conceptspaces.site_phase','diary',true); end if;
...
as $$
declare o site.observations%rowtype; m cde.models%rowtype; org_id uuid; model_id_value uuid:=nullif(input_payload->>'source_model_id','')::uuid; offline_id uuid:=nullif(input_payload->>'offline_package_id','')::uuid; number_value text; type_value text:=lower(coalesce(nullif(btrim(input_payload->>'observation_type'),''),'progress'));
begin
  if auth.uid() is null or not project.can_access_project(target_project_id) then raise exception 'project_access_required'; end if;
...
set search_path=public,site,cde,project,audit,auth,pg_temp
as $$
declare i public.inspection_records%rowtype; itp public.inspection_test_plans%rowtype; m cde.models%rowtype; n public.non_conformances%rowtype; itp_id_value uuid:=nullif(input_payload->>'itp_id','')::uuid; model_id_value uuid:=nullif(input_payload->>'source_model_id','')::uuid; offline_id uuid:=nullif(input_payload->>'offline_package_id','')::uuid; result_value text:=lower(btrim(input_payload->>'result')); org_id uuid;
begin
  if auth.uid() is null or not project.can_access_project(target_project_id) then raise exception 'project_access_required'; end if;
  ...
  if result_value='fail' then
    ...
    perform set_config('conceptspaces.site_phase','inspection_link',true);
...
as $$
declare pm site.progress_measurements%rowtype; b cost.boq_lines%rowtype; model cde.models%rowtype; boq_id uuid:=nullif(input_payload->>'boq_line_id','')::uuid; model_id_value uuid:=nullif(input_payload->>'source_model_id','')::uuid; org_id uuid;
begin
  if auth.uid() is null or not project.can_access_project(target_project_id) then raise exception 'project_access_required'; end if;
...
as $$
declare pm site.progress_measurements%rowtype; b cost.boq_lines%rowtype; prior_qty numeric; s text:=lower(btrim(target_decision)); org_id uuid;
begin
  select * into pm from site.progress_measurements where id=target_measurement_id for update;
  if not found or auth.uid() is null or not project.can_manage_project(pm.project_id) then raise exception 'progress_certification_authority_required'; end if;
  if pm.status<>'draft' or s not in ('certified','rejected') then raise exception 'progress_measurement_not_decidable'; end if;
  if s='certified' and pm.boq_line_id is not null then
    select * into b from cost.boq_lines where id=pm.boq_line_id;
    select coalesce(sum(measured_quantity),0) into prior_qty from site.progress_measurements where project_id=pm.project_id and boq_line_id=pm.boq_line_id and status='certified' and id<>pm.id;
    if prior_qty+pm.measured_quantity>b.quantity then raise exception 'progress_exceeds_approved_boq_quantity'; end if;
  end if;
  perform set_config('conceptspaces.site_phase','progress_certify',true);
  update site.progress_measurements set status=s,certified_by=case when s='certified' then auth.uid() else null end,certified_at=case when s='certified' then now() else null end where id=pm.id returning * into pm;
  select organisation_id into org_id from project.projects where id=pm.project_id;
  perform audit.append_event(org_id,pm.project_id,'site.progress.'||s,'progress_measurement',pm.id,null,to_jsonb(pm),target_reason,gen_random_uuid());
  return pm.status;
end;$$;
...
begin
  ...
  if nullif(btrim(input_payload->>'variation_ref'),'') is null or nullif(btrim(input_payload->>'description'),'') is null then raise exception 'variation_identity_required'; end if;
...
begin
  if auth.uid() is null or not project.can_access_project(target_project_id) then raise exception 'project_access_required'; end if;
  if jsonb_typeof(coalesce(input_payload->'evidence_refs','[]'::jsonb))<>'array' or jsonb_array_length(coalesce(input_payload->'evidence_refs','[]'::jsonb))=0 then raise exception 'PROGRESS_EVIDENCE_MISSING'; end if;
  if nullif(input_payload->>'contractor_id','') is null or nullif(input_payload->>'period_from','') is null or nullif(input_payload->>'period_to','') is null then raise exception 'progress_claim_identity_required'; end if;
...
set search_path=public,site,project,extensions,auth,pg_temp
as $$
declare p site.offline_packages%rowtype; snapshot jsonb; current_hash text; item jsonb; c site.offline_changes%rowtype; results jsonb:='[]'::jsonb; resource_id uuid;
begin
  ...
  for item in select value from jsonb_array_elements(target_changes) loop
    perform set_config('conceptspaces.site_phase','offline_sync',true);
    ...
    values(p.id,p.project_id,auth.uid(),btrim(item->>'local_change_id'),lower(btrim(item->>'entity_type')),coalesce(item->'payload','{}'::jsonb),coalesce(nullif(item->>'client_created_at','')::timestamptz,now()),'queued')
    ...
    resource_id:=null;
    if c.entity_type='site_diary' then resource_id:=public.record_site_diary(p.project_id,c.payload||jsonb_build_object('offline_package_id',p.id));
    ...
    perform set_config('conceptspaces.site_phase','offline_sync',true);
    update site.offline_changes set status='synced',server_resource_id=resource_id,synced_at=now(),conflict_reason=null where id=c.id;
  ...
  perform set_config('conceptspaces.site_phase','offline_sync',true);
  if p.source_hash<>current_hash or (p.expires_at is not null and p.expires_at<=now()) then update site.offline_packages set status=case when p.expires_at is not null and p.expires_at<=now() then 'expired' else 'stale' end where id=p.id; end if;
