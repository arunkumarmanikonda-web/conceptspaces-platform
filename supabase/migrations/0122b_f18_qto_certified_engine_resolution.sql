...
declare m cde.models%rowtype;q cost.qto_runs%rowtype;b cost.qto_benchmark_results%rowtype;org_id uuid;model_id_value uuid:=nullif(input_payload->>'model_id','')::uuid;rule_ref text:=btrim(input_payload->>'measurement_rule_set_ref');engine_ref_value text:=nullif(btrim(input_payload->>'engine_ref'),'');engine_version_value text:=nullif(btrim(input_payload->>'engine_version'),'');input_hash_value text;
begin
 ...
 if (engine_ref_value is null)<>(engine_version_value is null) then raise exception 'qto_engine_ref_and_version_must_be_supplied_together';end if;
 if engine_ref_value is null then
   select r.* into b from cost.qto_benchmark_results r join cost.qto_benchmark_cases bc on bc.id=r.case_id where r.status='approved' and r.passed and bc.status='approved' and bc.measurement_rule_set_ref=rule_ref order by r.reviewed_at desc,r.created_at desc limit 1;
   if b.id is null then raise exception 'QTO_BENCHMARK_CERTIFICATION_REQUIRED';end if;
   engine_ref_value:=b.engine_ref;engine_version_value:=b.engine_version;
 else
