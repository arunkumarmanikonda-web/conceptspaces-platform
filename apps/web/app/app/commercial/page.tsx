...
async function createProposal(formData:FormData){
  ...
  const payload={currency:"INR",valid_until:String(formData.get("valid_until")||""),tax:String(formData.get("tax")||"0"),commercial_notes:[],lines:[{title:String(formData.get("title")||"Professional Services"),scope_code:String(formData.get("scope_code")||"ARCH"),pricing_model:String(formData.get("pricing_model")||"fixed"),quantity:String(formData.get("quantity")||"1"),rate:String(formData.get("rate")||"0"),optional:false,sort_order:1}]};
...
async function transitionContract(formData:FormData){"use server";const {supabase}=await requireWorkspaceUser();const {error}=await supabase.rpc("transition_contract_state",{target_contract_id:String(formData.get("contract_id")||""),new_status:String(formData.get("status")||""),evidence_reference:String(formData.get("evidence_reference")||"")});if(error) throw new Error(error.message);revalidatePath("/app/commercial");}
...
function nextContractStates(status:string){
  if(status==="draft") return ["negotiation","signature_pending"];
  if(status==="negotiation") return ["signature_pending","terminated"];
  if(status==="signature_pending") return ["active","terminated"];
  if(status==="active") return ["suspended","completed","terminated"];
...
export default async function CommercialPage(){
  ...
  return <>
    ...
    <div className="panel-grid">
      <section className="panel"><h3>Contracts</h3><table className="table"><thead><tr><th>Contract</th><th>Status</th><th>Proposal</th><th>Transition</th></tr></thead><tbody>{contractRows.map(c=>{const states=nextContractStates(c.status);return <tr key={c.id}><td>{c.id.slice(0,8).toUpperCase()}</td><td><span className="badge">{c.status}</span></td><td>{c.proposal_id?`Proposal ${c.proposal_id.slice(0,8)}`:"—"}</td><td>{states.length>0?<form action={transitionContract} style={{display:"grid",gap:6,minWidth:180}}><input type="hidden" name="contract_id" value={c.id}/><select name="status">{states.map(s=><option key={s} value={s}>{s.replaceAll("_"," ")}</option>)}</select><input name="evidence_reference" placeholder="Signature/state evidence ref"/><button className="btn ghost" style={{padding:7,fontSize:9}}>Transition</button></form>:<span className="subtle">Terminal state</span>}</td></tr>})}{contractRows.length===0&&<tr><td colSpan={4} className="subtle">No contracts yet.</td></tr>}</tbody></table></section>
      <section className="panel"><h3>Create Invoice Draft</h3><form action={createInvoice}><div className="field"><label>Active Contract</label><select name="contract_id"><option value="">Unlinked invoice</option>{contractRows.filter(c=>c.status==="active").map(c=><option key={c.id} value={c.id}>{c.id.slice(0,8)} · active</option>)}</select></div><div className="field"><label>Invoice Number</label><input name="invoice_number" required placeholder="CS/26-27/001"/></div><div className="field-grid"><div className="field"><label>Issue Date</label><input type="date" name="issue_date"/></div><div className="field"><label>Due Date</label><input type="date" name="due_date"/></div><div className="field"><label>Description</label><input name="description" defaultValue="Professional services"/></div><div className="field"><label>Quantity</label><input type="number" min="0" step="0.01" name="quantity" defaultValue="1"/></div><div className="field"><label>Rate</label><input type="number" min="0" step="0.01" name="rate" required/></div><div className="field"><label>GST %</label><input type="number" min="0" step="0.01" name="gst_rate" defaultValue="18"/></div></div><button className="btn" style={{marginTop:16}}>Create Invoice Draft</button></form></section>
    </div>

    <section className="panel" style={{marginTop:16}}><h3>Invoices & Collections</h3><table className="table"><thead><tr><th>Invoice</th><th>Amount</th><th>Paid</th><th>Status</th><th>Due</th><th>Action</th></tr></thead><tbody>{invoiceRows.map(i=><tr key={i.id}><td>{i.invoice_number}</td><td>{money(i.total,i.currency)}</td><td>{money(i.amount_paid,i.currency)}</td><td><span className="badge">{i.status}</span></td><td>{new Date(i.due_date).toLocaleDateString("en-IN")}</td><td>{i.status==="draft"&&<form action={issueInvoice}><input type="hidden" name="invoice_id" value={i.id}/><button className="btn ghost" style={{padding:8}}>Issue</button>{i.created_by===user.id&&<div className="subtle">Independent checker required.</div>}</form>}</td></tr>)}{invoiceRows.length===0&&<tr><td colSpan={6} className="subtle">No invoices yet.</td></tr>}</tbody></table></section>
  </>;
}
